...
struct CMDPRGType
{
	...
	char Mode;
	char AxisNum;
	long int Move[AxisNumber];
	char Direction[AxisNumber];
	long int SpeedMa[AxisNumber];
	long int MaxSpeed;
	...
	switch(CMDPRGlist.Mode)
	{
		case 0:
		{
			...
			for (int ax = 0 ; ax < CMDPRGlist.AxisNum ; ax++)
			{
				...
				CMDPRGlist.SpeedMa[ax] = btol(b);
			}
			for (int ax = 0 ; ax < CMDPRGlist.AxisNum ; ax++)
			{
				CMDPRGlist.Direction[ax]=spie_master_tx_rx(0x00);
			}
